...
function resolveKey(layerName, style) {
  ...
  // 特殊处理 0：鞋子角度变体（鞋袜-左鞋角度1 / 鞋袜-右鞋角度2）
  // 格式：shoes-{l/r}-new-angle
  // 返回特殊标记 "ANGLE_VARIANT:xxx" 让 buildPlan 识别
  // 组结构和右鞋一致，但图层名保持 shoes-x-new-angle
  if (layerName.indexOf("角度") !== -1) {
    if (layerName.indexOf("左鞋") !== -1 || layerName.indexOf("鞋袜-左") !== -1) {
      return "ANGLE_VARIANT:shoes-l-new-angle";
    }
    if (layerName.indexOf("右鞋") !== -1 || layerName.indexOf("鞋袜-右") !== -1) {
      return "ANGLE_VARIANT:shoes-r-new-angle";
    }
  }
...
function buildPlan(style) {
  ...
  for (var i = 0; i < all.length; i++) {
    ...
    if (key) {
      // 特殊处理：角度变体（左鞋用 shoes-l 的 slot/skin，右鞋用 shoes-r 的 slot/skin）
      if (key.indexOf("ANGLE_VARIANT:") === 0) {
        var angleKey = key.substring("ANGLE_VARIANT:".length); // shoes-l-new-angle 或 shoes-r-new-angle

        // 根据左右决定使用哪个 mapping
        var mappingKey = angleKey.indexOf("shoes-l") === 0 ? "shoes-l" : "shoes-r";
        var mapping = PART_MAP[mappingKey];
        ...
        if (mapping) {
          plan.push({
            ...
            original: layer.name,
            isAngleVariant: true,
            slotGroup: "[slot]" + mapping.slot,  // slot-shoes-l 或 slot-shoes-r
            skinGroup: "[skin]" + mapping.skin + "-" + style,  // foot/foot-风格名
            attachment: angleKey + "-" + style  // shoes-l-new-angle-风格名 或 shoes-r-new-angle-风格名
          });
        }
        continue;
      }
...
function previewPlan(style) {
  ...
  var mirrorCount = 0;
  var angleVariantCount = 0;
  for (var i = 0; i < r.plan.length; i++) {
    var p = r.plan[i];
    if (p.isMirror) mirrorCount++;
    if (p.isAngleVariant) angleVariantCount++;

    msg += p.original;
    if (p.isMirror) msg += "   [镜像]";
    if (p.isAngleVariant) msg += "   [角度变体]";
    msg += "\n";
  ...
  if (angleVariantCount > 0) {
    msg += "其中角度变体 " + angleVariantCount + " 个（使用右鞋的 slot/skin 结构）\n";
  }
...
function organize(style) {
  ...
  for (var i = 0; i < r.plan.length; i++) {
    ...
    try {
      ...
      // 2) 创建 slot 组（文档根部）
      // 每个附件都创建独立的 slot 组，允许同名 slot 并存
      // 导出到 Spine 后，同名 slot 会自动合并
      var slotG = doc.layerSets.add();
